...
public class MainClass {
	...
	public MainClass(int ts, int tfs, int ecgs, double kkgo)
	{
		...
		for(int i=0;i<ts;i++)
		{
			Takim t=new Takim(i, getRandom(ilceler)+" "+getRandom(takimEkAd));
			for(int j=0;j<tfs;j++)
			{
				Futbolcu f=new Futbolcu(fi++, getRandom(adlar)+" "+getRandom(soyadlar));
				t.futbolcular.add(f);
			}
			takimlar.add(t);
		}
...
/*		for(Mac m :fikstur)
		{
			for(Gol g:m.goller)
			{
				System.out.println(m.hafta+" "+m.EvSahibi.id+" "+m.Deplasman.id+" "+g.dakika+" "+g.atanTakim.id+" "+g.atanFutbolcu.id);
			}
		}*/
	}
	/*
	...
	{
		try {
		FileOutputStream tkmlar=new FileOutputStream("output\\takimlar.txt");
		FileOutputStream ftblclr=new FileOutputStream("output\\futbolcular.txt");
		for(Takim t:takimlar)
		{
			...
			{
				String text2=Integer.toString(f.id)+" "+Integer.toString(t.id)+" "+f.adi+"\n";
		...
		for(Mac m:fikstur)
		{
			String str=Integer.toString(m.hafta)+" "+Integer.toString(m.EvSahibi.id)+" "+Integer.toString(m.Deplasman.id)+"\n";
			maclar.write(str.getBytes());
			for(Gol g:m.goller)
			{
				String str2=Integer.toString(m.hafta)+" "+Integer.toString(g.dakika)+" "+Integer.toString(g.atanTakim.id)+" "+Integer.toString(g.atanFutbolcu.id)+"\n";
				goller.write(str2.getBytes());
			}
		}
		goller.close();
		maclar.close();
		FileOutputStream db=new FileOutputStream("output\\dosyabilgileri.txt");
	...
	public void fiksturOlustur()
	{
		...
		for(int i=0;i<flength;i++)
			fikstur.add(new Mac(mi++, fikstur.get(i).Deplasman, fikstur.get(i).EvSahibi));
		//fikstur.add(new Mac(fikstur.get(i).hafta+takimlar.size()-1, fikstur.get(i).Deplasman, fikstur.get(i).EvSahibi));
/*		for(Mac m: fikstur)
 ...
 */	}
	...
	public void maclariOynat(int maxGoals, double ownGoalRate)
	{
		for(Mac m:fikstur)
		{
			...
			for(int i=0;i<homeScore;i++)
			{
				...
				{
					m.goller.add(new Gol(m.EvSahibi.getRandomPlayer(), m.EvSahibi, random.nextInt(91)));
				}
				else//own goal
				{
					m.goller.add(new Gol(m.Deplasman.getRandomPlayer(), m.EvSahibi,  random.nextInt(91)));
				}
			}
			for(int i=0;i<awayScore;i++)
			{
				if(random.nextDouble()>ownGoalRate)
				{
					m.goller.add(new Gol(m.Deplasman.getRandomPlayer(), m.Deplasman, random.nextInt(91)));
				}
				else//own goal
				{
					m.goller.add(new Gol(m.EvSahibi.getRandomPlayer(), m.Deplasman,  random.nextInt(91)));
				}
			}
			//m.goller.sort(c);
	...
	public static void main(String[] args) {
		...
		MainClass mc=new MainClass(takimSayisi, futbolcuSayisi, enCokGol, kkgo);
